...
def create_tenant_admin_user(event, context):
    ...
    if tenant_details["dedicatedTenancy"] == "true":
        ...
        user_pool_domain_response = user_mgmt.create_user_pool_domain(
            user_pool_id, tenant_id
        )
    ...
    tenant_user_mapping_response = user_mgmt.create_user_tenant_mapping(
        tenant_admin_user_name, tenant_id
    )
...
def create_user(event, context):
    ...
    if auth_manager.isSystemAdmin(user_role):
        ...
        tenant_details = table_tenant_details.get_item(Key={"tenantId": user_tenant_id})
        logger.info(tenant_details)
    ...
    if auth_manager.isTenantAdmin(user_role) or auth_manager.isSystemAdmin(user_role):
        ...
        user_mgmt = UserManagement()
        user_mgmt.add_user_to_group(
            user_pool_id, user_details["userName"], user_tenant_id
        )
        ...
        logger.log_with_tenant_context(
            event,
            "Request completed as unauthorized. Only tenant admin or system admin can create user!",
        )
...
def get_user(event, context):
    ...
    if auth_manager.isTenantUser(user_role) and user_name != requesting_user_name:
        logger.log_with_tenant_context(
            event,
            "Request completed as unauthorized. User can only get its information.",
        )
        return utils.create_unauthorized_response()
    else:
...
def update_user(event, context):
    ...
    if auth_manager.isTenantUser(user_role):
        logger.log_with_tenant_context(
            event,
            "Request completed as unauthorized. Only tenant admin or system admin can update user!",
        )
        return utils.create_unauthorized_response()
    else:
        user_info = get_user_info(event, user_pool_id, user_name)
...
def disable_user(event, context):
    ...
    if auth_manager.isTenantAdmin(user_role) or auth_manager.isSystemAdmin(user_role):
        ...
        logger.log_with_tenant_context(
            event,
            "Request completed as unauthorized. Only tenant admin or system admin can disable user!",
        )
        return utils.create_unauthorized_response()
...
def disable_users_by_tenant(event, context):
    ...
    if (
        auth_manager.isTenantAdmin(user_role)
        and tenantid_to_update == requesting_tenant_id
        ...
        for user in users:
            response = client.admin_disable_user(
                Username=user["userName"], UserPoolId=tenant_user_pool_id
            )
        ...
        logger.info(
            "Request completed as unauthorized. Only tenant admin or system admin can update!"
        )
        return utils.create_unauthorized_response()
...
class UserManagement:
    ...
    def create_user_pool_domain(self, user_pool_id, tenant_id):
        response = client.create_user_pool_domain(
            Domain=tenant_id + "-serverlesssaas", UserPoolId=user_pool_id
        )
        return response
    ...
    def create_tenant_admin(self, user_pool_id, tenant_admin_user_name, user_details):
        response = client.admin_create_user(
            ...
            ForceAliasCreation=True,
            UserAttributes=[
                {"Name": "email", "Value": user_details["tenantEmail"]},
                {"Name": "email_verified", "Value": "true"},
    ...
    def create_user_tenant_mapping(self, user_name, tenant_id):
        response = table_tenant_user_map.put_item(
            Item={"tenantId": tenant_id, "userName": user_name}
        )
