...
def create_tenant(event, context):
    ...
    table_tenant_details = dynamodb.Table(
        "ServerlessSaaS-TenantDetails"
...
def update_tenant(event, context):
    ...
    if (
        ...
        if (
            exiting_tenant_details["Item"]["tenantTier"].upper()
...
def deactivate_tenant(event, context):
    ...
    if (
        ...
        if response["Attributes"]["dedicatedTenancy"].upper() == "TRUE":
            ...
            update_user_response = __invoke_deprovision_tenant(
                update_details, headers, auth, host, stage_name, url_deprovision_tenant
            )
...
def activate_tenant(event, context):
    ...
    if auth_manager.isSystemAdmin(user_role):
        ...
        if response["Attributes"]["dedicatedTenancy"].upper() == "TRUE":
            update_details = {}
            update_details["tenantId"] = tenant_id
            provision_response = __invoke_provision_tenant(
                update_details, headers, auth, host, stage_name, url_provision_tenant
            )
...
def load_tenant_config(event, context):
    ...
    try:
        ...
        if response["Count"] == 0:
            return utils.create_notfound_response(
                "Tenant not found."
...
def __invoke_deprovision_tenant(
    update_details, headers, auth, host, stage_name, invoke_url
):
    try:
        url = "".join(
            ["https://", host, "/", stage_name, invoke_url + update_details["tenantId"]]
        )
        response = requests.put(
            url, data=json.dumps(update_details), auth=auth, headers=headers
        )
