...
create_tenant_admin_user_resource_path = os.environ[
...
def register_tenant(event, context):
    try:
        ...
        if (
            tenant_details["tenantTier"].upper()
            == utils.TenantTier.PLATINUM.value.upper()
        ):
        ...
        create_user_response = __create_tenant_admin_user(
            tenant_details, headers, auth, host, stage_name
        )
        ...
        tenant_details["tenantAdminUserName"] = create_user_response["message"][
            "tenantAdminUserName"
        ]

        create_tenant_response = __create_tenant(
            tenant_details, headers, auth, host, stage_name
        )
        logger.info(create_tenant_response)

        if tenant_details["dedicatedTenancy"].upper() == "TRUE":
            provision_tenant_response = __provision_tenant(
                tenant_details, headers, auth, host, stage_name
            )
...
def __create_tenant_admin_user(tenant_details, headers, auth, host, stage_name):
    try:
        url = "".join(
            ["https://", host, "/", stage_name, create_tenant_admin_user_resource_path]
        )
        logger.info(url)
        response = requests.post(
            url, data=json.dumps(tenant_details), auth=auth, headers=headers
        )
        response_json = response.json()
    except Exception as e:
        logger.error("Error occured while calling the create tenant admin user service")
        raise Exception(
            "Error occured while calling the create tenant admin user service", e
        )
    else:
...
def __provision_tenant(tenant_details, headers, auth, host, stage_name):
    try:
        url = "".join(
            ["https://", host, "/", stage_name, provision_tenant_resource_path]
        )
        logger.info(url)
        response = requests.post(
            url, data=json.dumps(tenant_details), auth=auth, headers=headers
