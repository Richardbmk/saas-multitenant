...
def do_action(event, context):
    ...
    For Pooled tenants it is shared and for Silo (Platinum tier tenants) it
    is unique to them.
    This method keeps the URL for Pooled tenants Settings Table, since
    it is shared across multiple tenants,
    And for Silo tenants inside the tenant management table along with other
    tenant settings, for that tenant
    ...
    if tenant_id.lower() == "pooled":
        ...
        response = tenant_details.update_item(
            ...
            ExpressionAttributeValues={":apiGatewayUrl": tenant_api_gateway_url},
            ReturnValues="NONE",
        )
